...
class TestVisibility(unittest.TestCase):
    ...
    def _t(self):
        import io as _io, json as _json, os as _os, re as _re
        return _re.sub(r"<[^>]+>", "", xrs.render_digest(_json.load(_io.open(_os.path.join(_os.path.dirname(__file__),'..','data','latest.json'),encoding='utf-8'))))

    def test_no_line_exceeds_mobile_width(self):
        for ln in self._t().split("\n"):
            self.assertLessEqual(xrs.vis_width(ln), xrs.LINE_COLS, ln)
